...
-- USERS Policies
drop policy if exists "Users can read profiles of everyone" on public.users;
...
-- DOCUMENTS Policies
drop policy if exists "Users can read General documents or documents in their department" on public.documents;
...
drop policy if exists "Admins and uploaders can manage documents" on public.documents;
...
drop policy if exists "Users can read chunks of documents they have access to" on public.document_chunks;
...
drop policy if exists "Admins and service roles can modify chunks" on public.document_chunks;
...
drop policy if exists "Users can manage messages in their own chat sessions" on public.chat_messages;
...
drop policy if exists "Admins view all feedback" on public.message_feedback;
...
drop policy if exists "Admins manage review queue" on public.flagged_responses;
create policy "Admins manage review queue" on public.flagged_responses
  for all using (
    (select role from public.users where id = auth.uid()) = 'admin'
  );

drop policy if exists "Users view own flagged items" on public.flagged_responses;
create policy "Users view own flagged items" on public.flagged_responses
  for select using (auth.uid() = user_id);

-- RETRIEVAL LOGS Policies
drop policy if exists "Admins view retrieval logs" on public.retrieval_logs;
create policy "Admins view retrieval logs" on public.retrieval_logs
  for select using (
    (select role from public.users where id = auth.uid()) = 'admin'
  );

drop policy if exists "Authenticated users insert retrieval logs" on public.retrieval_logs;
